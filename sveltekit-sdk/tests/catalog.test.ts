...
it('runs shared filter presets through the actual Node evaluator', async () => {
  ...
  await client.init();
  for (const f of filters) client.state.definitions.set(f.featureKey, f);
  for (const f of filters) {
    ...
    expect(nonmatching, f.featureKey).toBe(
      ['filter-always-on', 'filter-time-window'].includes(f.featureKey),
    );
  }
  await client.close();
});
