...
<svelte:head
  ><title>Toggly · SvelteKit feature flags</title><meta
    name="description"
    content="Explore request-scoped feature flags, server actions, hydration and entity contexts with Toggly."
  /></svelte:head
>
<div class="shell">
  <aside>
    <a class="brand" href="/">toggly<span> / SvelteKit</span></a>
    <p class="eyebrow">FEATURE FLAGS IN PRACTICE</p>
    <nav>
      {#each sections as section}<a
          href="/{section === 'home' ? '' : section}?preset={data.matching
            ? 'matching'
            : 'non-matching'}">{section[0].toUpperCase() + section.slice(1)}</a
        >{/each}
    </nav>
    <p class="footnote">Node adapter · Svelte 5<br />Request → snapshot → browser</p>
  </aside>
  <main>
    {#if data.offline}<div class="banner" role="status">
