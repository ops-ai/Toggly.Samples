...
{#if section === "home"}
  <p>
    Explore a complete feature flag journey: evaluate once with request context,
    hydrate the same result, then receive signed browser updates.
  </p>
  <div class="card">
    <h2>Start with your first toggle</h2>
    <p>
      Configure <code>new-dashboard</code> as Available to Client SDK. Switch it ON
      in Production and watch the enabled card appear in Declarative. Switch it OFF
      and the fallback replaces it. With empty keys, this sample uses the labelled
      offline fixture instead.
    </p>
  </div>
  <div class="card">
    ...
    <p>
      Allowlisted frontend values only. Raw backend rules and backend App Keys
  ...
  <div class="card">
    ...
    {#each sections.slice(1) as target}<p>
        <a href="/{target}">{target}</a> — an executable view with the matching
        source in <code>src/routes/[[section]]/+page.svelte</code>.
      </p>{/each}
  </div>
{:else if section === "declarative"}
  <p>
    Feature gates select visible content. These boolean branches are not
  ...
  <Feature {toggly} feature="api-v2" options={{ negate: true }}
    ><div class="card">
      Negate: API v1 remains visible while api-v2 is off.
    </div></Feature
  >
  ...
  <div class="card">
    ...
    <p>
      This SDK exposes boolean and entity gates. It does not assign A/B
      variants; do not label ON and OFF as experiment groups or invent a variant
      identifier.
    </p>
  </div>
{:else if section === "programmatic"}
  ...
  <p>
    Use the same layout-owned store in event handlers and derived UI values.
  </p>
  <button on:click={() => localGate.toggle()}>Toggle device prerequisite</button
  >
  <p>
    Device ready: {String(localReady)}. Local gates cannot enable a remotely
    disabled flag.
  </p>
  <div class="card">
...
{:else if section === "identity"}
  ...
  <p>
    The server hook creates one immutable context per request. Changing the demo
    preset navigates with new server data; the layout replaces its browser
    session.
  </p>
  <form method="GET">
  ...
  <p>
    Demo role claims are not authentication. Production claims come from your
    authenticated server session; expose only public targeting attributes.
  </p>
{:else if section === "entity"}
  ...
  <p>
    Express Checkout evaluates the same flag with two Order instances; a global
    boolean would lose this distinction.
  </p>
  <div class="card">
    <table>
      <thead
        ><tr
          ><th>Entity</th><th>Vip</th><th>Server result</th><th
            >Browser result</th
          ></tr
        ></thead
      ><tbody
        >{#each [vip, standard] as order}<tr
            ><td>{order.key}</td><td>{String(order.attributes.Vip)}</td><td
              >{String(
                order === vip ? data.vipEnabled : data.standardEnabled,
              )}</td
            ><td
              >{String(
                toggly.isEnabled("ExpressCheckout", { entity: order }),
              )}</td
            ></tr
          >{/each}</tbody
      >
    ...
    <p>
      Offline frontend defaults omit ExpressCheckout, so its browser result is
      false until live frontend definitions supply the entity gate. Server
      fixture results use the real Node evaluator.
    </p>
  </div>
{:else if section === "filters"}
  <p>
    These results use the server's request-bound Node evaluator. Browser segment
    results use the browser's actual headers after hydration.
  </p>
  <form method="GET">
    ...
    <table>
      <thead><tr><th>Flag</th><th>Evaluated result</th></tr></thead><tbody
        >{#each data.matrix as row}<tr
            ><td><code>{row.key}</code></td><td
              class:on={row.enabled}
              class:off={!row.enabled}>{row.enabled ? "ON" : "OFF"}</td
            ></tr
          >{/each}</tbody
      >
    </table>
    <p>
      AlwaysOn and the open TimeWindow remain ON for both presets. Percentage is
      sticky by identity; its result is not prescribed by the preset name.
    </p>
  </div>
{:else if section === "framework"}
  ...
  <p>
    Hooks bind context, the framework route load requires beta-access, and
    actions enforce enhanced-submit again before work.
  </p>
  <div class="card">
    ...
    <p>
      Turn enhanced-submit off to receive HTTP 404 from the action. Keep your
      existing authorization check alongside the feature gate.
    </p>
  </div>
  <div class="card">
    <h2>Lifecycle</h2>
    <p>
      The layout starts refresh and a WebSocket in onMount; onDestroy stops its
      polling and socket. Navigation replaces the request snapshot immediately
      and rejects results from the previous browser session.
    </p>
    <p>
      Initial support is adapter-node. Prerender captures build-time defaults;
      personalized pages and server actions require a running server.
    </p>
  </div>
