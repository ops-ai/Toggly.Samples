...
with (root / 'target' / 'smoke.log').open('w') as log:
    process = subprocess.Popen([java, '-jar', 'target/java-spring-webflux-sdk.jar'], cwd=root, env=env, stdout=log, stderr=log)
    try:
        for attempt in range(100):
            if process.poll() is not None:
                raise RuntimeError('Packaged process exited before startup')
            try:
                urllib.request.urlopen('http://localhost:8089/', timeout=1).close()
                break
