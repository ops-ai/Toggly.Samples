...
public final class PackagedProbe {
    public static void main(String[] args) throws Exception {
        ...
        if (!source.endsWith("java-servlet-sdk.jar")) throw new AssertionError("Application must load from packaged JAR: " + source);
        try (var definitions = new DefinitionsServer(); var app = Main.start(8086, definitions.lifecycle()); var http = HttpClient.newHttpClient()) {
            for (String path : new String[]{"/", "/gates", "/programmatic", "/identity", "/orders", "/filters", "/servlet", "/configuration", "/style.css", "/gated/beta?preset=matching", "/api/features?preset=matching"}) {
                ...
                if (path.startsWith("/api/features") && !response.body().contains("\"filter-targeting\":true")) throw new AssertionError("native context missing");
            }
            ...
            if (args.length > 0 && args[0].equals("--serve")) {
                System.out.println("TEST FIXTURE browser server on http://localhost:8086 (no live Toggly app)");
