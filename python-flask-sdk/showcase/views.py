...
from toggly_flask import (get_toggly, feature_flag_required, feature_gate_required,
    feature_flag_switch, feature_variant, FeatureFlagBlueprint)
...
def snapshot():
    ...
    return {'identity': context.identity, 'shared_identity': client.current_identity,
        ...
        'orders': {name: client.is_enabled('ExpressCheckout', context=context.with_entity(entity))
            for name,entity in [('ord-vip',order(True)),('ord-standard',order(False)),('missing',None),
...
def home():
    ...
    return render_template('home.html', snapshot=snapshot(), initialized=client.is_initialized,
        last_refresh=debug.last_refresh, refresh_error=bool(debug.last_error), interval=debug.refresh_interval)

@pages.get('/api/snapshot/')
def api_snapshot(): return snapshot()

@pages.get('/gates/')
def gates():
    return render_template('gates.html', any_gate=g.toggly.evaluate_gate(['new-dashboard','api-v2'], 'any'),
...
def programmatic():
    return render_template('programmatic.html', native=g.toggly.is_enabled('new-dashboard'),
...
def identity():
    if request.method=='POST':
        ...
        else: abort(400, 'Choose alice, bob, or clear.')
...
def orders():
    if request.method=='POST':
        ...
        if mode not in {'vip','standard','missing'}: abort(400,'Choose a listed Order.')
        session['order']=mode
        return redirect('/orders/')
    return render_template('orders.html', orders=snapshot()['orders'], native=g.toggly.is_enabled('ExpressCheckout'))

@pages.route('/filters/', methods=['GET','POST'])
def filters():
    if request.method=='POST':
        preset=request.form.get('preset')
        if preset not in {'matching','nonmatching'}: abort(400,'Choose a listed preset.')
        login_user(USERS['alice' if preset=='matching' else 'bob'])
        session['preset']=preset;session['order']='vip' if preset=='matching' else 'standard'
        return redirect('/filters/')
    rows=[{'key':key,'alias':alias,'note':note,'enabled':get_toggly().is_enabled(key,context=g.sample_context),
        'native':g.toggly.is_enabled(key)} for key,alias,_,note in FILTERS]
    return render_template('filters.html',rows=rows)

@pages.get('/integrations/')
def integrations(): return render_template('integrations.html')

@pages.post('/submit/')
@feature_flag_required('enhanced-submit')
def submit():
    # Server gate precedes the reversible session mutation; CSRF is still active.
...
def targeted(): return result('Native targeted view: alice allowed')

@pages.get('/native/all/')
@feature_gate_required(['new-dashboard','api-v2'])
def all_gate(): return result('Native All gate allowed')

@pages.get('/native/any/')
@feature_gate_required(['new-dashboard','api-v2'], requirement=FeatureRequirement.ANY)
def any_gate(): return result('Native Any gate allowed')

@pages.get('/native/negate/')
@feature_gate_required(['new-dashboard','api-v2'], negate=True)
def negated_gate(): return result('Native negated All gate allowed')

@pages.get('/native/redirect/')
@feature_flag_required('api-v2',redirect_url='/integrations/')
def redirect_gate(): return result('API v2 enabled')

@pages.get('/native/fallback/')
@feature_flag_required('api-v2',fallback_view=lambda:result('API v1 fallback view'))
def fallback_gate(): return result('API v2 enabled')

@feature_beta.route('/welcome/')
def beta_welcome(): return result('Native FeatureFlagBlueprint: beta welcome')

pages.add_url_rule('/native/switch/', 'switch', feature_flag_switch('api-v2',
    enabled_view=lambda:result('API version 2'), disabled_view=lambda:result('API version 1')))

@pages.get('/native/variant/')
@feature_variant('new-dashboard', default_view=lambda:result('Native variant default: feature OFF'),
...
def native_variant():
    ...
    return result('Enabled; no named native variant', message='Use the explicit remote assignment action on Declarative gates. The native decorator cannot read the assigned name from core metadata.')

@pages.post('/refresh/')
def refresh():
    response=get_toggly().client.refresh()
    return result(f'Native refresh status: {response.status.value}', message='Reload a section for a new evaluation. Error details and app keys are not displayed.')
...
def variant():
    ...
    config=TogglyConfig(app_key=current_app.config['TOGGLY_APP_KEY'],
        environment=current_app.config['TOGGLY_ENVIRONMENT'],base_url=current_app.config['TOGGLY_BASE_URL'],
        use_signed_definitions=True, enable_variants=True, identity=context.identity,
    ...
    with TogglyClient(config) as client:
        ...
        return result('Native remote assignment', variant=asdict(assigned) if assigned else None,
