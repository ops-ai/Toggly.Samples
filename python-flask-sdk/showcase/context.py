...
def order(vip):
    # An entity describes the business object being evaluated, not its user.
    # The shared flag recipe checks this Order's Vip property for checkout.
...
def prepare_context():
    ...
    # Supply the entity before any native helper can cache its request context.
    # Flask-Login independently resolves the user from this request's session.
...
def template_context():
    ...
    return {
        'persona': context.identity if context and context.identity else 'anonymous',
