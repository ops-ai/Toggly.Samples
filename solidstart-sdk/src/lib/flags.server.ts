...
const backend = createTogglyClient({
  ...
  baseUrl: process.env.TOGGLY_BASE_URL,
...
export async function requestScope(matching: boolean, request = getRequestEvent()!.request) {
  ...
  return createTogglyRequest({
    ...
    frontend: {
      ...
      environment: process.env.VITE_TOGGLY_ENVIRONMENT ?? 'Production',
      baseURI: process.env.VITE_TOGGLY_BASE_URL,
      expose: keys,
      flagDefaults: defaults,
    },
  });
}
export async function loadFlags(matching: boolean) {
  const scope = await requestScope(matching);
  try {
    return {
      snapshot: await scope.snapshot(),
      backendConfigured: Boolean(process.env.TOGGLY_BACKEND_APP_KEY),
    };
  } finally {
    scope.dispose();
  }
