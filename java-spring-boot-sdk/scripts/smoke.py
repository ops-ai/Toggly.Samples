...
env = dict(os.environ, TOGGLY_APP_KEY='', TOGGLY_ENABLED='false')
with (root / 'target' / 'smoke.log').open('w') as log:
    process = subprocess.Popen([java, '-jar', 'target/java-spring-boot-sdk.jar'], cwd=root, env=env, stdout=log, stderr=subprocess.STDOUT)
    try:
        for attempt in range(100):
            if process.poll() is not None:
                raise RuntimeError('Packaged process exited before startup')
            try:
                urllib.request.urlopen('http://127.0.0.1:8090/', timeout=1).close()
                break
            except (OSError, urllib.error.URLError):
                time.sleep(0.1)
        else:
            raise RuntimeError('Packaged process did not start')
        for path in ['/', '/gates', '/programmatic', '/identity', '/orders', '/filters', '/boot', '/configuration']:
            with urllib.request.urlopen('http://127.0.0.1:8090' + path) as response:
        ...
        for path in ['/gated/feature', '/gated/negate', '/gated/all', '/gated/any', '/gated/beta', '/native/argument']:
            try:
                urllib.request.urlopen('http://127.0.0.1:8090' + path)
                raise AssertionError('Unconfigured protected/native route unexpectedly allowed: ' + path)
            except urllib.error.HTTPError as error:
                assert error.code == 503
        print('Packaged smoke: eight pages rendered, five gates and native AOP example denied')
    finally:
        process.terminate()
        try:
