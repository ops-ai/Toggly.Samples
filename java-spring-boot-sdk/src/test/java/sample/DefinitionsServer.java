...
final class DefinitionsServer implements AutoCloseable {
    ...
    DefinitionsServer() throws Exception {
        ...
        jwks = "{\"keys\":[{\"kty\":\"EC\",\"crv\":\"P-256\",\"alg\":\"ES256\",\"use\":\"sig\",\"kid\":\"" + kid
                + "\",\"x\":\"" + base64.encodeToString(x) + "\",\"y\":\"" + base64.encodeToString(y) + "\"}]}";
        publish(true);
    ...
    Map<String, Object> properties(long pollSeconds) {
        Map<String, Object> props = new HashMap<>();
        props.put("toggly.enabled", true);
        props.put("toggly.app-key", "offline-test-placeholder");
        props.put("toggly.environment", "Production");
        props.put("toggly.base-url", url());
        props.put("toggly.refresh-interval-seconds", pollSeconds);
        props.put("toggly.enable-usage-tracking", false);
        props.put("toggly.enable-metrics", false);
        props.put("toggly.register-contexts-on-startup", false);
        return props;
    }
    ...
    void publish(boolean dashboard) throws Exception {
        envelope = signed(definitions(dashboard), ++timestamp);
    }
    ...
    String signed(String defs, long ts) throws Exception {
        ...
        var signer = Signature.getInstance("NONEwithECDSA");
        signer.initSign(key.getPrivate());
        signer.update(hash);
        return "{\"defs\":" + defs + ",\"timestamp\":" + ts + ",\"kid\":\"" + kid + "\",\"signature\":\""
    ...
    static String definitions(boolean dashboard) {
        ...
        definitions.add(flag("filter-time-window", "TimeWindow",
    ...
    private static String flag(String key, String name, String params) {
        if (Set.of("UserClaims", "Country", "BrowserFamily", "BrowserLanguage", "DeviceType", "OperatingSystem")
                .contains(name)) {
