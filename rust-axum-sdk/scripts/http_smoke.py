...
def run(output: Path):
    ...
    with (output / "production-http.log").open("w") as log:
        process = subprocess.Popen(
            [str(ROOT / "target/release/rust-axum-sdk-sample")],
        ...
        try:
            ...
            for _ in range(150):
                if process.poll() is not None:
                    raise RuntimeError("Axum exited before startup")
                try:
                ...
                raise RuntimeError("Axum startup timeout")
            ...
            result = {
                "production_host": "Axum 0.7.9 / Rust 1.98.1",
                "published_crates": "toggly 0.6.0 / toggly-axum 0.6.0",
                "workers": 8,
...
if __name__ == "__main__":
    ...
    if args.output:
        ...
        with tempfile.TemporaryDirectory(prefix="axum-sample-smoke-") as temporary:
            run(Path(temporary))
