// Flag keys are case-sensitive identifiers shared by the code and definitions.
// They select rules, not fixed booleans: the same key can differ by environment,
// request identity, claims, or Order. Keep these aligned with FLAG_TEMPLATE.md.
...
// Presets supply controlled targeting inputs, including a simulated browser.
// Eight filter rows switch with these presets. AlwaysOn and the open TimeWindow
// stay on; Percentage hashes identity and flag key, so its outcome is not forced.
...
// Ready entity objects avoid global entity state. kind selects the definition
// context; attributes.Vip is the boolean read by the fixture ContextProperty rule.
export const orders = {
...
// Explicit query values win over preset/header fallbacks. Unknown orders use
// ord-standard; these small demo records are not loaded from a trusted database.
// anonymous is a shared fallback, not a generated browser/session identifier.
