...
import { closeHonoToggly, getHonoToggly } from '@ops-ai/toggly-hono';
...
async function serve(config, run) {
  ...
  try { await run((path, options) => app.request(path, options)); }
  finally { closeHonoToggly(); }
}
test('actual published adapter/core HTTP integration', async t => {
  const fixture = await startFixture();
  try {
    await serve({ fixtureUrl: fixture.baseUrl, contextHook: async c => { await new Promise(resolve => setTimeout(resolve, c.req.header('x-toggly-identity') === 'alice' ? 7 : 1)); } }, async request => {
      ...
      await t.test('all filter rows and 30 simultaneous request identities, claims, Order and HTTP headers', async () => {
        const globalIdentity = getHonoToggly().identity;
        ...
        assert.equal(getHonoToggly().identity, globalIdentity);
      });
      await t.test('middleware gates, negate, all/any and unique handlers', async () => {
        ...
        assert.equal((await request('/unique/route', { method: 'POST' })).status, 200);
      ...
      await t.test('failed refresh preserves last known definitions and reports cached provenance', async () => {
        fixture.state.status = 503; await getHonoToggly().refresh();
    ...
    await t.test('configured signed mode rejects unsigned transport and never claims live', async () => {
      ...
      try {
        await serve({ appKey: 'test-only-not-a-real-key', telemetry: false }, async request => {
          ...
          assert.ok(getHonoToggly().state.error);
        });
...
test('context extraction error retains fixture provenance on Hono error response', async () => {
  const fixture = await startFixture();
  try {
    await serve({ fixtureUrl: fixture.baseUrl, contextHook() { throw new Error('authentication lookup failed'); } }, async request => {
      const response = await request('/api/evaluate');
      assert.equal(response.status, 503);
      assert.match(response.headers.get('x-toggly-source'), /^Offline fixture/);
      assert.match((await response.json()).source, /^Offline fixture/);
    });
  } finally { await fixture.close(); }
});

test('real Node listener and fetch smoke', async () => {
  const { serve: listen } = await import('@hono/node-server');
  closeHonoToggly();
  const fixture = await startFixture();
  const app = createApp({ fixtureUrl: fixture.baseUrl });
  const server = listen({ fetch: app.fetch, hostname: '127.0.0.1', port: 0 });
  await new Promise(resolve => server.once('listening', resolve));
  try {
    const response = await fetch(`http://127.0.0.1:${server.address().port}/api/evaluate?preset=matching`);
    assert.equal(response.status, 200);
    assert.match(response.headers.get('x-toggly-source'), /^Offline fixture/);
    assert.equal((await response.json()).flags.ExpressCheckout, true);
  } finally {
