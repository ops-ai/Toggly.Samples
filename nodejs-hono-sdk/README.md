...
1. Sign into [Toggly](https://app.toggly.io), select **Toggly Samples**, create **Node Hono SDK Sample**, choose the matching Hono/Node technology and **Production** environment. Allow `http://localhost:3000`.
...
- [ ] Apply Matching/Non-matching on filters; verify the nine prescribed rows, AlwaysOn/TimeWindow and sticky percentage.
...
The installed Hono 0.3.0 runtime and types differ from the docs example that passes a second argument to context helpers: extra arguments are ignored. The core-client override shown here follows the installed artifact. `featuresHandler` is a handler value, not a factory. `featureRoutes` supports method filtering; GET `/unique/route` is gated while POST demonstrates the explicit GET-only policy. `getHonoToggly()` exposes the singleton for lifecycle diagnostics/tests, never request identity mutation.
