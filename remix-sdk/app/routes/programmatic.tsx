...
export async function loader(args: LoaderFunctionArgs) {
  ...
  return sampleLoader().run(args, async ({ client }: { client: any }) => json({ configured: true, single: await client.isEnabled('new-dashboard'), any: await client.evaluateGate(['new-dashboard', 'api-v2'], 'any'), negated: await client.evaluateGate(['new-dashboard', 'api-v2'], 'all', true) }))
}

export async function action(args: LoaderFunctionArgs) {
  if (!hasServerKey()) return json({ accepted: false, reason: 'Add TOGGLY_APP_KEY before testing an action gate.' }, { status: 503 })
  return sampleAction().run(args, async (_request: unknown, toggly: { isEnabled: (key: string) => Promise<boolean>; context: { identity?: string } }) => json({ accepted: await toggly.isEnabled('enhanced-submit'), identity: toggly.context.identity || 'anonymous' }))
}
