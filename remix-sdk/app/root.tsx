...
import { APP_KEY, ENVIRONMENT, hasServerKey, sampleLoader } from './lib/toggly.server'
import './styles.css'
...
export default function App() {
  ...
  return <html lang="en"><head><meta charSet="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1" /><Meta /><Links /></head><body><RemixTogglyProvider fallbackContext={data.serverContext} config={{ appKey: data.publicKey, environment: data.environment }}><Layout><MissingKeyBanner publicKey={data.publicKey} serverKey={data.serverKey} /><Outlet /></Layout></RemixTogglyProvider><ScrollRestoration /><Scripts /></body></html>
}
