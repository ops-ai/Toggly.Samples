...
type Options = { configured: boolean; environment?: string; telemetryEnabled?: boolean }
...
export function mountSample(root: Element, sdk: SampleSdk, options: Options) {
  ...
  function render() {
    ...
    // numeric public API here; the package exposes this through its browser global.
    ...
    <section id="telemetry"><h2>7. Frontend telemetry</h2><p id="telemetry-status" aria-live="polite"></p><p>Feature checks are collected by the SDK as this sample evaluates flags. Rendering content alone does not record a view; use the explicit controls below.</p><button id="telemetry-evaluate">Evaluate new-dashboard</button> <strong id="telemetry-check-result"></strong><p><button data-telemetry-write id="telemetry-usage">Record usage</button> <button data-telemetry-write id="telemetry-view">Record view</button> <button data-telemetry-write id="telemetry-counter">Increment sample-actions</button> <button data-telemetry-write id="telemetry-gauge">Set sample-cart-size</button> <button data-telemetry-write id="telemetry-flush">Flush telemetry</button></p><p id="telemetry-action-status" aria-live="polite"></p></section><section id="unique"><h2>8. Package-unique surfaces</h2><p>Browser global, supported refresh hook, WebSocket refresh, variants, offline defaults, and entity registration.</p></section><section id="configuration"><h2>9. Configuration</h2><p id="configuration-text"></p></section></main>`
    if (options.configured) root.querySelector('#missing-key')?.remove()
    text('#evaluation-status', status); text('#result-label', resultLabel)
    const telemetryAvailable = options.configured && options.telemetryEnabled !== false
    text('#telemetry-status', !options.configured
      ? 'Offline mode: checks use local defaults and telemetry is silent.'
      : telemetryAvailable
        ? 'Telemetry is enabled for this configured app. The SDK captures evaluations automatically; explicit events below are sent only after you click their controls.'
        : 'Telemetry is opted out in SDK configuration. Feature evaluation remains available.')
    root.querySelectorAll<HTMLButtonElement>('[data-telemetry-write]').forEach(button => { button.disabled = !telemetryAvailable })
    ...
    root.querySelector('#telemetry-evaluate')?.addEventListener('click', () => {
      const enabled = sdk.isFeatureOn('new-dashboard')
      const variant = sdk.getVariant('new-dashboard')
      text('#telemetry-check-result', `new-dashboard ${enabled ? 'ON' : 'OFF'}; variant ${variant?.name ?? '(none)'}`)
      text('#telemetry-action-status', 'The SDK records this feature check automatically when telemetry is enabled.')
    })
    root.querySelector('#telemetry-usage')?.addEventListener('click', () => {
      sdk.recordUsage('new-dashboard', sdk.getVariant('new-dashboard')?.name ?? 'enabled')
      text('#telemetry-action-status', 'Usage event added to the SDK telemetry batch.')
    })
    root.querySelector('#telemetry-view')?.addEventListener('click', () => {
      sdk.recordView('new-dashboard', sdk.getVariant('new-dashboard')?.name ?? 'enabled')
      text('#telemetry-action-status', 'View event added to the SDK telemetry batch.')
    })
    root.querySelector('#telemetry-counter')?.addEventListener('click', () => {
      sdk.incrementCounter('sample-actions', 1)
      text('#telemetry-action-status', 'Counter event added to the SDK telemetry batch.')
    })
    root.querySelector('#telemetry-gauge')?.addEventListener('click', () => {
      sdk.setGauge('sample-cart-size', 3)
      text('#telemetry-action-status', 'Gauge event added to the SDK telemetry batch.')
    })
    root.querySelector('#telemetry-flush')?.addEventListener('click', () => {
      void sdk.flushTelemetry()
        .then(() => text('#telemetry-action-status', 'Flush completed; telemetry delivery is best effort.'))
        .catch(() => text('#telemetry-action-status', 'Flush could not be completed.'))
    })
  }
