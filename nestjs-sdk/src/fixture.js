...
const rule = (featureKey, name, parameters = {}) => ({
  featureKey,
...
export const definitions = [
  ...['new-dashboard', 'enhanced-submit', 'beta-access', 'filter-always-on'].map((key) =>
    rule(key, 'AlwaysOn'),
  ),
  rule('api-v2', 'AlwaysOff'),
  entity('ExpressCheckout'),
  entity('filter-context-property'),
  ...
  rule('filter-time-window', 'TimeWindow', {
    Start: '2020-01-01T00:00:00Z',
    End: '2099-12-31T23:59:59Z',
  }),
  ...[
    ['country', 'Country', 'US'],
    ['browser-family', 'BrowserFamily', 'Chrome'],
    ['browser-language', 'BrowserLanguage', 'en'],
    ['device-type', 'DeviceType', 'Macintosh'],
    ['os', 'OperatingSystem', 'Mac'],
  ].map(([key, name, value]) =>
    rule(`filter-${key}`, name, { [`${name}:0`]: value, Percentage: 100 }),
  ),
];
