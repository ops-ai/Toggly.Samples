...
export function inputs(req) {
  ...
  const value = (key, fallback) =>
    typeof req.query[key] === 'string' ? req.query[key].slice(0, 500) : fallback;
  return {
    identity: value('identity', p?.identity ?? req.get('x-toggly-identity') ?? 'anonymous'),
    role: value('role', p?.role ?? req.get('x-demo-role') ?? 'user'),
    order: orders[value('order', p?.order ?? req.get('x-demo-order'))] ?? orders['ord-standard'],
    preset: p,
  };
}
