...
import { getFastifyToggly } from '@ops-ai/toggly-fastify';
...
async function serve(config, run) {
  assert.equal(getFastifyToggly(), null, 'previous instance cleaned up');
  const app = await createApp(config);
  await app.listen({ port: 0, host: '127.0.0.1' });
  const request = (path, options) => fetch(`${app.listeningOrigin}${path}`, options);
  try { await run(request); } finally { await app.close(); assert.equal(getFastifyToggly(), null, 'onClose clears client'); }
}

test('actual published adapter/core HTTP integration', async t => {
  const fixture = await startFixture();
  try {
    await serve({ fixtureUrl: fixture.baseUrl, hooks: [{ getMetadata: () => ({ name: 'async-isolation' }), beforeEvaluation: async () => { await new Promise(resolve => setImmediate(resolve)); } }] }, async request => {
      ...
      await t.test('all filter rows and 30 simultaneous request identities, claims, Order and HTTP headers', async () => {
        const globalIdentity = getFastifyToggly().identity;
        ...
        assert.equal(getFastifyToggly().identity, globalIdentity);
      });
      await t.test('preHandler gates, negate, all/any and unique handlers', async () => {
        for (const [path, status] of [['/gates/enabled', 200], ['/gates/negate', 200], ['/gates/all', 404], ['/gates/any', 200], ['/unique/route', 404], ['/unique/wrapped', 200]]) assert.equal((await request(path)).status, status, path);
      ...
      await t.test('failed refresh preserves last known definitions and reports cached provenance', async () => {
        fixture.state.status = 503; await getFastifyToggly().refresh();
    ...
    await t.test('configured signed mode rejects unsigned transport and never claims live', async () => {
      ...
      try {
        await serve({ appKey: 'test-only-not-a-real-key', telemetry: false }, async request => {
          ...
          assert.ok(getFastifyToggly().state.error);
        });
        assert.ok(definitionRequests > 0);
      } finally { globalThis.fetch = originalFetch; }
    });
    await t.test('async context extraction error retains offline labels on error responses', async () => {
      fixture.state.status = 200;
      await serve({ fixtureUrl: fixture.baseUrl, getContext: async () => { throw new Error('demo context failure'); } }, async request => {
