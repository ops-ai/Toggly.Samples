...
import io.toggly.spring.mvc.ContextResolver;
import jakarta.servlet.http.HttpServletRequest;

/** Resolve complete inputs before the native interceptor installs them on this synchronous worker. */
public final class DemoContextResolver implements ContextResolver {
    ...
    @Override public EvaluationContext resolve(HttpServletRequest req) {
        ...
        return EvaluationContext.builder().identity(chosen.identity().isBlank() ? null : chosen.identity())
                ...
                .entity("missing".equals(req.getParameter("order")) ? null : Catalog.order(vip)).build();
    }
