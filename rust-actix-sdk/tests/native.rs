...
use cookie::Key;
use rust_actix_sdk_sample::{
    Mode, application_with_key,
...
impl Session {
    ...
    fn store(&mut self, headers: &actix_web::http::header::HeaderMap) {
...
async fn exchange<S, B>(
    app: &S,
    session: &mut Session,
    mut builder: TestRequest,
) -> (
    StatusCode,
...
async fn workshop() -> (
    ...
    >,
    Fixture,
    web::Data<TogglyClient>,
) {
    context::register_order();
    let fixture = Fixture::start().unwrap();
    let client = web::Data::new(TogglyClient::new(fixture.config(false)).await.unwrap());
    let app = test::init_service(application_with_key(
        Some(client.clone()),
        Mode {
            offline: true,
...
async fn token<S, B>(app: &S, session: &mut Session) -> String
where
    S: Service<actix_http::Request, Response = ServiceResponse<B>, Error = actix_web::Error>,
    B: MessageBody,
{
    let (_, _, body) = exchange(app, session, TestRequest::get().uri("/identity")).await;
...
async fn preset<S, B>(app: &S, session: &mut Session, matching: bool)
where
    S: Service<actix_http::Request, Response = ServiceResponse<B>, Error = actix_web::Error>,
    B: MessageBody,
{
...
#[actix_web::test]
async fn gates_native_extractor_middleware_guard_and_missing_data() {
    let (app, fixture, core) = workshop().await;
    ...
    let (status, _, _) = exchange(&app, &mut Session::new(), TestRequest::get().uri("/beta")).await;
    assert_eq!(status, StatusCode::OK);
    let (status, _, _) =
        exchange(&app, &mut Session::new(), TestRequest::get().uri("/layer")).await;
    assert_eq!(status, StatusCode::OK);
    let (status, _, _) =
        exchange(&app, &mut Session::new(), TestRequest::get().uri("/guard")).await;
    assert_eq!(status, StatusCode::OK);
    let (status, _, body) =
        exchange(&app, &mut Session::new(), TestRequest::get().uri("/state")).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(
    ...
    let (status, _, _) =
        exchange(&app, &mut Session::new(), TestRequest::get().uri("/layer")).await;
    assert_eq!(status, StatusCode::NOT_FOUND);
    // FeatureGuard is existence, not evaluation: AlwaysOff but still defined.
    let (status, _, _) =
        exchange(&app, &mut Session::new(), TestRequest::get().uri("/guard")).await;
    assert_eq!(status, StatusCode::OK);
    let mut session = Session::new();
    ...
    let (status, _, _) = exchange(
        ...
        TestRequest::post()
            .uri("/submit")
            .insert_header((header::CONTENT_TYPE, "application/x-www-form-urlencoded"))
            .set_payload(format!("csrf={csrf}")),
    )
    .await;
    assert_eq!(status, StatusCode::FORBIDDEN);

    tokio::time::sleep(Duration::from_secs(1)).await;
    let mut removed = Fixture::definitions();
    removed
        .as_array_mut()
        .unwrap()
        .retain(|definition| definition["featureKey"] != "beta-access");
    fixture.replace(removed, false);
    core.refresh().await.unwrap();
    let (status, _, _) =
        exchange(&app, &mut Session::new(), TestRequest::get().uri("/guard")).await;
    assert_eq!(status, StatusCode::NOT_FOUND);

    let missing = test::init_service(application_with_key(
        None,
        Mode {
            offline: false,
            startup_failed: true,
    ...
    let (status, _, _) = exchange(
        &missing,
        &mut Session::new(),
        TestRequest::get().uri("/beta"),
    )
    .await;
    assert_eq!(status, StatusCode::SERVICE_UNAVAILABLE);
    let (status, _, body) =
        exchange(&missing, &mut Session::new(), TestRequest::get().uri("/")).await;
    assert_eq!(status, StatusCode::OK);
    assert!(
    ...
    let (status, _, _) = exchange(
        &missing,
        &mut Session::new(),
        TestRequest::get().uri("/layer"),
    )
    .await;
    // Published TogglyMiddleware allows the request through when client Data
    // is missing instead of failing closed.
    assert_eq!(status, StatusCode::OK);
    let (status, _, _) = exchange(
        &missing,
        &mut Session::new(),
        TestRequest::get().uri("/guard"),
    )
    .await;
    // FeatureGuard fails closed when client Data is missing (no route match).
    assert_eq!(status, StatusCode::NOT_FOUND);
}
...
async fn sessions_csrf_validation_escape_and_pages() {
    ...
    let mut session = Session::new();
    for (slug, _) in rust_actix_sdk_sample::views::SECTIONS {
        let path = format!("/{slug}");
        let (status, headers, body) =
            exchange(&app, &mut session, TestRequest::get().uri(&path)).await;
    ...
    let (status, _, _) = exchange(
        ...
        TestRequest::post()
            .uri("/session")
            .insert_header((header::CONTENT_TYPE, "application/x-www-form-urlencoded"))
            .set_payload("csrf=wrong&action=matching&destination=home"),
    )
    .await;
    assert_eq!(status, StatusCode::FORBIDDEN);
    let csrf = token(&app, &mut session).await;
    ...
    .await;
    assert_eq!(status, StatusCode::SEE_OTHER);
    let (_, _, body) = exchange(&app, &mut session, TestRequest::get().uri("/identity")).await;
    let html = String::from_utf8(body.to_vec()).unwrap();
    ...
    for matching in [true, false, true] {
        ...
        let (_, _, body) = exchange(
            &app,
            &mut session,
            TestRequest::get().uri("/api/check?key=ExpressCheckout"),
        )
        .await;
...
async fn signed_tampering_transport_failure_refresh_and_shutdown() {
    ...
    // Published 0.6.1 rejects a signed document whose timestamp is not newer.
    tokio::time::sleep(Duration::from_secs(1)).await;
