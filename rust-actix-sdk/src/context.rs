...
pub fn load_jar(cookie_header: Option<&str>, key: &Key) -> CookieJar {
...
pub fn persona_from_jar(jar: &CookieJar, key: &Key) -> Persona {
    jar.private(key)
        .get("persona")
        .and_then(|cookie| serde_json::from_str::<Persona>(cookie.value()).ok())
        .filter(|persona| order(&persona.order).is_some())
        .unwrap_or_default()
}

pub fn csrf_from_jar(jar: &mut CookieJar, key: &Key) -> String {
    if let Some(cookie) = jar.private(key).get("csrf") {
...
pub fn save_persona(jar: &mut CookieJar, key: &Key, persona: &Persona) {
    // AES-GCM private cookies hide workshop choices from the browser. They are
    // still not real user privileges; production apps must derive identity from
...
impl CookieJarHandle {
    pub fn save(&self, persona: &Persona) {
        let mut jar = self.jar.lock().expect("cookie jar");
        save_persona(&mut jar, &self.key, persona);
    }
}
...
impl FromRequest for WorkshopContext {
    type Error = Error;
    ...
    fn from_request(req: &HttpRequest, _payload: &mut Payload) -> Self::Future {
        ready(
            req.extensions()
                .get::<WorkshopContext>()
                .cloned()
                .ok_or_else(|| ErrorInternalServerError("missing workshop context")),
        )
    }
}

impl FromRequest for CookieJarHandle {
    type Error = Error;
    type Future = Ready<Result<Self, Self::Error>>;
    ...
    fn from_request(req: &HttpRequest, _payload: &mut Payload) -> Self::Future {
        ready(
            req.extensions()
                .get::<CookieJarHandle>()
                .cloned()
                .ok_or_else(|| ErrorInternalServerError("missing cookie jar")),
        )
    }
}
