...
<head>
    ...
    <title>{title} · Rust Actix SDK Workshop</title>
    <link rel="stylesheet" href="/static/workshop.css">
</head>
<body>
<a class="skip" href="#content">Skip to content</a>
<header>
    <div class="eyebrow">TOGGLY / RUST + ACTIX-WEB 4.15</div>
    <h1>Feature flags, one request at a time.</h1>
    <p>A hands-on workshop using published toggly 0.6.1, toggly-actix 0.6.1 and Actix-web 4.15.0.</p>
</header>
...
<footer>Rust 1.98.1 · One shared client · Request-local context · Demo personas are not authentication</footer>
</body>
</html>
"##
    )
...
async fn declarative(context: &WorkshopContext, client: Option<&TogglyClient>) -> String {
    ...
    <p>Actix has no Toggly template directive. This sample declares keys, a requirement and negate, calls native <code>evaluate_gate</code>, then chooses HTML.</p>
    ...
    <p>Published 0.6.1 negates each individual feature before Any/All aggregation. It does not invert the final gate. An empty key list returns false. Errors render the fallback.</p>
</section>
<div class="grid">
{cards}</div>
<section class="card">
    <h3>Variants: unsupported</h3>
    <p>Published toggly 0.6.1 has no native variant allocation API. This sample does not invent a variant or call a boolean result a variant.</p>
</section>"##
    )
}
...
fn identity(context: &WorkshopContext) -> String {
    ...
    <p>Workshop middleware reads AES-GCM private cookies, resolves a persona and builds a new EvalContext before any SDK check. It never writes identity into the process-wide client.</p>
...
async fn orders(context: &WorkshopContext, client: Option<&TogglyClient>) -> String {
    ...
    <p>Register the Order schema locally before client construction. In published 0.6.1 the remote schema PUT happens during construction, before the first definition fetch, and is best-effort; provision the context and flag binding manually as described in Configuration.</p>
</section>"##,
        result = badge(&result)
    )
}
...
fn surfaces() -> String {
    r##"<section class="card">
    <h3>Native Actix Feature extractor</h3>
    <pre><code>async fn native(feature: toggly_actix::Feature) {
    let enabled = feature.is_enabled("beta-access").await;
    // feature.context() exposes the header-only native context.
}</code></pre>
    <p><a href="/native">Try the native extractor</a>. It reads X-User-Id first, then X-Identity; absent headers are anonymous. Those headers are untrusted demo inputs. The adapter does not map workshop cookies, claims, groups, request metadata or Order.</p>
    <pre><code>curl -H 'X-User-Id: alice' -H 'X-Identity: bob' http://localhost:8018/native</code></pre>
    <p>The extractor returns false on evaluation error. If <code>web::Data&lt;TogglyClient&gt;</code> is absent, published <code>Feature</code> and <code>TogglyData</code> <strong>panic</strong>. This workshop checks app data first and returns HTTP 500 instead. <code>FeatureEnabled</code> is a public data struct in 0.6.1 without <code>FromRequest</code>, so it cannot be used as a native handler extractor.</p>
    <h3>TogglyMiddleware, TogglyData and app data</h3>
    <p><a href="/layer">Open the middleware beta route</a>. <code>TogglyMiddleware::with_feature("beta-access").identity_header("X-User-Id")</code> evaluates that one header only and returns HTTP 404 when the flag is off. Evaluation errors become false. There are no <code>require</code> or <code>deny</code> factory methods. If the client Data is missing, the published middleware <strong>allows the request through</strong>.</p>
    <p><a href="/extractor">TogglyData</a> unwraps the same <code>web::Data&lt;TogglyClient&gt;</code>. <a href="/state">App data</a> is the published Actix state surface; this demo uses <code>EvalContext::default()</code>.</p>
    <h3>FeatureGuard is definition existence, not evaluation</h3>
    <p><a href="/guard">Open the guarded route</a>. <code>FeatureGuard::new("beta-access")</code> calls <code>is_defined_sync</code>. A defined flag whose condition is false still matches. <code>FeatureGuard::disabled</code> inverts that existence check; the name does not mean asynchronous off-state evaluation. Missing client data fails the guard (no route match / 404).</p>
    ...
    <p>The process owns one <code>web::Data&lt;TogglyClient&gt;</code> cloned into Actix workers. Graceful shutdown calls <code>close</code> once. No per-request client, identity mutation, definition refresh or cache clear occurs. The default 60-second evaluation key omits Order, claims and request metadata. This workshop uses public <code>cache_ttl(Duration::ZERO)</code> so those edits expire immediately. Entries are still created; this is not a structural cache-disable switch.</p>
</section>"##.into()
}
...
fn setup() -> String {
    ...
    <ol>
        <li>Create Rust Actix SDK Sample in a workspace you can manage, technology Rust, environment Production.</li>
    ...
    <p>This server evaluates locally. Browser origins are only needed if you introduce direct browser SDK requests; when applicable add http://localhost:8018 (and http://127.0.0.1:8018 if used).</p>
