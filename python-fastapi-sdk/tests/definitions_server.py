...
class DefinitionsServer:
    def __init__(self):
        ...
        # The verifier retrieves this public key by the envelope's kid. The
        # private key stays only in this fixture process for generating payloads.
        self.jwks = {
            'keys': [
                {
                    'kty': 'EC',
        ...
        self.variant_started = threading.Event()
        self.variant_delay = 0
        owner = self
                    ...
                    if self.path == '/.well-known/jwks':
                        ...
                        if self.path.startswith('/evaluated-variants-signed/'):
                            ...
                            data[0]['filters'] = [
                                {
                                    'name': (
                                        'AlwaysOn' if owner.dashboard else 'AlwaysOff'
                                    ),
                                    'parameters': {},
                                },
                            ]
                        ...
                        # Match the wire signing input exactly: compact JSON,
                        # timestamp, nested SHA-256 and the native ES256 envelope.
                        digest = hashlib.sha256(
                            hashlib.sha256(f'{raw}|{timestamp}'.encode()).digest()
                        ).digest()
                        signature = owner.key.sign(
                            digest,
