...
for name, pin in direct.items():
    assert locked_normalized.get(normalize(name)) == pin, (
        f'Direct pin mismatch: {name}'
    )
...
for spec in project['project']['dependencies']:
    name, pin = spec.split('==')
    assert locked_normalized[normalize(name)] == pin, (
        f'Wheel dependency mismatch: {name}'
    )

print(
    f'LOCK_OK direct={len(direct)} installed={len(locked)} '
