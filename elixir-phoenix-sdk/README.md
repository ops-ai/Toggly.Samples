...
| `TOGGLY_SNAPSHOT_PATH` | Optional writable file path for signed definitions and accepted public signing keys in connected mode. Unset disables persistence. Use a durable application-owned directory and a separate file per application/environment; restart after changes. Ignored in unsigned demonstration mode. |
...
`Showcase.Application` supervises one fixed `Showcase.Flags` client. Its GenServer owns definitions, timers and protected ETS state; each evaluation reads the local snapshot. Connected mode starts with explicit false defaults for the four baseline keys, then restores a valid configured file snapshot before fetching. Without a valid restored snapshot, an unavailable first fetch leaves defaults in place; later errors preserve last-known-good definitions. Unknown keys default false. Refresh failure does not force existing active flags off.

Online definitions use ES256 signatures by default. Persistence is disabled when `TOGGLY_SNAPSHOT_PATH` is unset. Set it to retain the signed envelope and accepted public signing keys fetched from the HTTPS origin for verified offline restarts; independently configured trusted JWKS override the persisted keys. See [Offline restart with live definitions](#offline-restart-with-live-definitions) and the [Elixir SDK docs](https://docs.toggly.io/sdks/elixir). WebSockets invalidate definitions; polling provides a fallback. Stop/restart is owned by the application supervisor.

Set `TOGGLY_MAX_SIGNATURE_AGE_SECONDS=86400`, for example, to reject signed envelopes older than one day during refresh or file restoration (the exact boundary is allowed). Rejected refreshes preserve active verified flags and their ETag; this is not a timer that turns active flags off. A cold start retains defaults if no valid snapshot or sufficiently fresh, valid signed response is available. A configured snapshot older than the limit is rejected on offline startup, so choose the limit with your expected outage duration in mind. Future-skew and active-timestamp rollback checks still apply even when age checking is disabled. This sample's explicit unsigned offline fixture mode is unaffected.
