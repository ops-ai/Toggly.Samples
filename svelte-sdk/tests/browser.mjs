...
const servers = [];
const buildDirectory = await mkdtemp(join(tmpdir(), "toggly-svelte-browser-"));
...
try {
  ...
  if (process.env.SCREENSHOT_DIR) {
    ...
    await page.screenshot({
      path: `${process.env.SCREENSHOT_DIR}/svelte-desktop.png`,
      fullPage: true,
    });
    await page.setViewportSize({ width: 390, height: 844 });
    await page.screenshot({
      path: `${process.env.SCREENSHOT_DIR}/svelte-mobile.png`,
      fullPage: true,
    });
  }
  ...
  assert.ok(mainPacket, "the createToggly-owned main service sends explicit events");
