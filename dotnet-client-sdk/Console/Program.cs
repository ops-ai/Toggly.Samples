...
showcase.Client.Changed += (_, _) =>
    Console.WriteLine("Flags/context changed; open Home to inspect.");
...
try
{
    ...
    if (args.Contains("--smoke"))
    {
        ...
        {
            Console.WriteLine(showcase.Render(section));
        }
    ...
    while (!shutdown.IsCancellationRequested)
    {
        ...
        {
            await showcase.SetPresetAsync(command == "m", shutdown.Token);
        }
        else if (command == "l")
        {
        ...
        {
            await showcase.Client.RefreshAsync(shutdown.Token);
        }
        else if (
            int.TryParse(command, out var index)
            && index >= 1
            && index <= Showcase.Sections.Length
        )
        {
            Console.WriteLine(showcase.Render(Showcase.Sections[index - 1]));
        }
    }
}
catch (OperationCanceledException) when (shutdown.IsCancellationRequested)
{
    // Ctrl+C ends the host after the SDK drains outstanding refreshes.
}
